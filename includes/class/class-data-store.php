 ...
 * Copied from WooCommerce WC_Data_Store class and modified a little.
 * Original file: https://github.com/woocommerce/woocommerce/blob/master/includes/class-wc-data-store.php
 *
 * @package AnsPress
 * @subpackage Classes
 * @since 4.2.0
...
class Data_Store {
	 ...
	 * Example: 'question' => 'AnsPress\Question_Data_Store_CPT'
	 ...
	 * Ran through `ap_data_stores`.
	 *
	 * @var array
	 */
	private $stores = array(
		'question' => 'AnsPress\Question_Data_Store_CPT',
	);
	...
	public function __construct( $object_type ) {
		...
		$this->stores      = apply_filters( 'ap_data_stores', $this->stores );
		...
		if ( array_key_exists( $object_type, $this->stores ) ) {
			$store = apply_filters( 'ap_' . $object_type . '_data_store', $this->stores[ $object_type ] );

			if ( is_object( $store ) ) {
				if ( ! $store instanceof WC_Object_Data_Store_Interface ) {
					throw new \Exception( __( 'Invalid data store.', 'anspress-question-answer' ) );
				}
